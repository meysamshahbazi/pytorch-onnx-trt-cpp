...
void preprocessImage(const string &image_path,float * gpu_input,const nvinfer1::Dims &dims)
{
    ...
    resized.convertTo(flt_image,CV_32FC3,1.0f/255.0f);
...
void postprocessResults1(float * gpu_output,const nvinfer1::Dims &dims, int batch_size)
{
    auto classes = getClassNames("../imagenet_classes.txt");

    vector<float> cpu_output(getSizeByDim(dims)*batch_size);
    cudaMemcpy(cpu_output.data(),gpu_output,cpu_output.size()*sizeof(float),cudaMemcpyHostToDevice);
